...
async def prof_access(page: Page, id_interno: int, role: str = 'Instructor') -> None:
    ...
    print(f'Professores bloqueados encontrados : {len(user_name)} na sala {id_interno}')
    
    if len(user_name) > 0:
        classUrlUltra = f'./ultra/courses/{id_interno}/outline'
        await page.goto(url=classUrlUltra, wait_until='domcontentloaded')
...
async def coord(page: Page, id_interno: int, user: str) -> None:
