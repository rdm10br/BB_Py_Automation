from playwright.async_api import Page, expect

async def removeSer(page: Page, id_interno: str, item: str = 'SER Melhor (Clique Aqui para deixar seu elogio, crítica ou sugestão)') -> None:
    
    classurl = f'./ultra/courses/{id_interno}/outline?search={item}'
    await page.goto(url=classurl, wait_until='commit')
    await page.wait_for_timeout(1000*4)
    await page.wait_for_load_state('networkidle')
    await page.wait_for_load_state('domcontentloaded')
    await page.wait_for_load_state('load')
    await page.wait_for_timeout(1000*6)
    if await page.get_by_role("heading", name="Nenhum resultado encontrado").is_visible(timeout=3*1000):
        print(f'{item} not found')
        pass
    else:
        print(f'{item} found')
        await page.get_by_label(f'Mais opções para {item}').click()
        await page.wait_for_load_state('load')
        await page.get_by_role('menuitem', name='Excluir').click()
        await page.wait_for_load_state('load')
        await page.get_by_role('button', name='Excluir').click()
        await page.wait_for_load_state('load')
        await page.wait_for_timeout(1000*3)
        await expect(page.get_by_role("heading", name="Nenhum resultado encontrado")).to_be_visible(timeout=3*1000)