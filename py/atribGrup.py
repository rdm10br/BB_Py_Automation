...
def inserirArquivo(playwright: Playwright , id_interno) -> None:
    browser = playwright.chromium.connect_over_cdp("http://localhost:9222")
    context = browser.contexts[0]
    page = context.pages[1]
    baseURL = "https://sereduc.blackboard.com/"
    classURL = baseURL+"ultra/courses/"
    importgroup = f"https://sereduc.blackboard.com/webapps/bb-group-mgmt-LEARN/jsp/groupspace/ex/ImportGroups.jsp?course_id={id_interno}&toggleType=all&fromPage=groups"
    file_path = 'py\\GRUPOS1.csv'
    file_path_sem_fael = 'py\\GRUPOS_SEM_FAEL.csv'
    
    page.goto(importgroup)
    # page.set_input_files("#arg_file_groups_chooseLocalFile",file_path) # arquivo para o digital
    page.set_input_files("#arg_file_groups_chooseLocalFile",file_path_sem_fael) #arquivo para o veteranos
...
def atribuirGrupos(playwright: Playwright , id_interno) -> None:
    browser = playwright.chromium.connect_over_cdp("http://localhost:9222")
    context = browser.contexts[0]
    ...
    page.goto(classURL+id_interno+"/groups")
