    ...
    def run(self):
        try:
            ...
            if self.script_path == r'src\Metodos\Login\getCredentials.py':
                ...
                # if src_dir not in sys.path:
                #     sys.path.insert(0, src_dir)
                # from Decorators.Main_StartUp import Worker_startup
                
                # index =  Worker_startup().do_work()
                
                for line in iter(process.stdout.readline, ''):
                    print(line.strip())
                    progress = self.parse_progress_from_output(line.strip())
                    self.progress_updated.emit(progress)
                    # self.progress_updated.connect(self.main_window.update_progress_bar(index))
                    # self.message_box_signal.emit(line.strip())
